...
using namespace std;
int cx(int x)//size manager, every position given and every size given has to pass through this
{
	return x;
...
public:
	...
	List<abs_pos> makeList(List<coords> ob) //makes a list of continous coordinates in x and y using the row and column information.
	{
		List<abs_pos> l;
	...
	List<abs_pos> makeTrajectory(double x1, double y1, double x2, double y2);//function to return the list of having the pixel points for the bullet
...
protected:
	///functions capable of changing the ship attributes will be written here in protected mode, later these functions will be collectively called under one public function protected with an ID
	bool isAuthenticPath(List<Greed::abs_pos> ob)//function to check if the list supplied is authentic or not
	{
...
private:
	...
	public:
	Greed::abs_pos getRealAbsolutePosition()//returns the abs_pos of the left most corner of the rectangle
	{
		unique_lock<mutex> lk(mutx->m[ship_id]);
...
public:
	...
	*/
	...
	bool checkCollision(int sid,const Greed::bullet &ob)//sid is the ship id of  the victim ship.
	{
		...
		Greed::abs_pos coord(bullet_pos.left-origin_x, bullet_pos.top-origin_y);
		
		 if (dir == Direction::EAST || dir == Direction::WEST)
		{
			 
			if (coord.x >= entity.c1.x && coord.x <= entity.c2.x && coord.y >= entity.c1.y + 10 && coord.y <= entity.c4.y - 10)
			{
				return true;
			}
			else
		 ...
		 else
		 {
			 ...
			 if ((coord.y >= entity.c1.y && coord.y <= entity.c4.y) && (coord.x >= entity.c1.x + 10 && coord.x <= entity.c2.x - 10))
			 {
				 return true;
			 }
			 else
...
double shipInfo::getCurrentResources()
{
	return ob->getCurrentResources();
}
...
attribute ship::whatsHere(Greed::coords ob, int m)//pass the row and column of the tile to get whats there
{
	...
	*/
	...
	else if (m == 1)//call the localMap
	{
		//
		graph = localMap;
		cout << "\n inside the function==>" << graph[10 * columns + 0].b.cost;
	}
	attribute att;
	Control cont;
	List<Greed::coords> bonus = cont.getBonusList();
	List<Greed::coords> storm = cont.getStormList();
	...
	{
		if (::getIndex<Greed::coords>(storm, ob) != -1)
		{
			att.entity = Entity::STORM;
			att.cost = STORM_COST;

		}
		else if (::getIndex<Greed::coords>(bonus, ob) != -1)
		{

			att.entity = Entity::BONUS;
			att.cost = WATER_COST;
		}
		else
		{
			att.cost = WATER_COST;
			att.entity = Entity::WATER;
		}
	}
...
int Greed::cannon::getVictimShip()
{
	...
	for (int i = 0; i < ship_list.size(); i++)
	{
		...
		{
			if (isShipInMyRadius(ship_list[i]->getShipId(), ShipSide::FRONT))
			{
...
public:
	...
	public:
		...
		void list1_renderer()
		{
			...
			list1->getRenderer()->setScrollbarWidth(10);
			tgui::Scrollbar sc;
			...
			list1->setHeaderTextSize(20);
			list1->addColumn("RANK", 100, tgui::ListView::ColumnAlignment::Center);
			list1->addColumn("PLAYER_NAME", 240, tgui::ListView::ColumnAlignment::Center);
			list1->addColumn("SCORE", 100, tgui::ListView::ColumnAlignment::Center);
			list1->addColumn("SHIPS DEFEATED", 240, tgui::ListView::ColumnAlignment::Center);
			list1->addColumn("SURVIVAL TIME", 150, tgui::ListView::ColumnAlignment::Center);
		...
		void frameRateSlider_renderer()
		{
			...
			frameRateSlider->setSize(300, 10);
		...
		void gameOverRenderer(sf::Font &font)
		{
			...
			gameOver.setPosition(100, 350);//position of  the text
		...
		void winner_renderer(sf::Font& font)
		{
			...
			winner_text.setPosition(250,650);
			winner_text.setCharacterSize(100);
		}
		//define a menu_button renderer function
		...
		void closeButton_renderer(sf::Texture &tex)
		{
			...
			closeButton->setSize(tex.getSize().x,tex.getSize().y);
			closeButton->getRenderer()->setTexture(tex);
			closeButton->setPosition(400,5);
		...
		void time_line_renderer(tgui::Gui &gui)
		{
			for (int i = 0; i < no_of_players; i++)
			{
				...
				timeline->setSize(445, 430);
				timeline->setTextSize(18);
				...
				timeline->setHeaderTextSize(20);
				timeline->addColumn("Timestamp", 150, tgui::ListView::ColumnAlignment::Center);
				timeline->addColumn("Event", 240, tgui::ListView::ColumnAlignment::Center);
		...
		void stats_view_renderer()
		{
			...
			stats_view->setSize(450,350);
			stats_view->setTextSize(18);
			...
			stats_view->setHeaderTextSize(20);
			//adding the columns here
			stats_view->addColumn("S.No.",60);
			stats_view->addColumn("ATTRIBUTE",200,tgui::ListView::ColumnAlignment::Center);
			stats_view->addColumn("VALUE",150, tgui::ListView::ColumnAlignment::Center);
			//adding the values
		...
		void stats_view_panel_renderer(tgui::Group::Ptr g)
		{
			...
			stats_view_panel->setSize(450, 900);
			stats_view_panel->setPosition(1490, 200);
		...
		void doIt()
		{
			...
			stats_view->setPosition(100, 100);
		}
		...
		void menu_button_renderer(sf::Texture& tex,tgui::Gui &gui)
		{
			for (int i = 0; i < no_of_players; i++)
			{
				...
				p->setSize(tex.getSize().x, tex.getSize().y);
		...
		void setting_button_renderer(sf::Texture &tex1,sf::Texture &tex2,tgui::Gui &gui)
		{
			...
			settings->setSize(tex1.getSize().x, tex1.getSize().y);
		...
		void p_renderer()
		{
			...
			for (int i = 0; i < no_of_players; i++)
			{
				...
				player_panels[i]->setSize(240,168);
		...
		void score_renderer2()//for printing the score in realtime
		{
		...
		//write a function for rendering health_value without a for loop
		...
		//write fire_value_renderer function
		...
		//write gold value renderer function
		...
		void setScene(int w, int h, int xx, int yy, sf::Texture& tex, int code[rows][columns])//here xx and yy are the coordinates
		{
			...
			for (int i = 0; i < h; i++)
			{
				...
				for (int j = 0; j < w; j++)
				{
					...
					tile[z].position = sf::Vector2f(x, y);
					tile[z + 1].position = sf::Vector2f(x + len, y);
					temp[2].position = sf::Vector2f(x + len, y + len);
					temp[3].position = sf::Vector2f(x, y + len);

					temp[0].texCoords = sf::Vector2f(len * code[i][j], 0);
					temp[1].texCoords = sf::Vector2f(len * code[i][j] + len, 0);
					temp[2].texCoords = sf::Vector2f(len * code[i][j] + len, len);
					temp[3].texCoords = sf::Vector2f(len * code[i][j], len);
					if (showPatterns)
					{
	...
	public:
		...
		void frame(double elapsed_time, sf::Sprite& sp)
		{
			...
			//cout << "\n frame is==>" << frame;
			sp.setTextureRect(sf::IntRect(frame * width, 0, width, width));
	...
	void callable(Mutex *mutx, int code[rows][columns], Map& map_ob)//taking the ship object so as to access the list of the player
	{
		...
		//sf::Sprite rect[2];
		sf::Texture tex_s;
		tex_s.loadFromFile("ship_s.png");
		sf::Texture tex_n;//texture of north
		tex_n.loadFromFile("ship_n.png");
		sf::Texture tex_e;
		tex_e.loadFromFile("ship_e.png");
		sf::Texture tex_w;
		tex_w.loadFromFile("ship_w.png");
		sf::Texture tex_ne;
		tex_ne.loadFromFile("ship_ne.png");
		sf::Texture tex_nw;
		tex_nw.loadFromFile("ship_nw.png");
		sf::Texture tex_se;
		tex_se.loadFromFile("ship_se.png");
		sf::Texture tex_sw;
		tex_sw.loadFromFile("ship_sw.png");
		...
		{
			cannon_list[i].graphic_initializer(cannon_tex, sf::Vector2f(cannon_list[i].tile.c * 80 + 40 + origin_x, cannon_list[i].tile.r * 80 + 40 + origin_y));
		}


		for (int i = 0; i < 2; i++)//graphic initializer for the ship of the user
		{
		...
		*/
		...
		GuiRenderer gui_renderer(2,&window);
		...
		gui_renderer.player_name[0]->setText(pl1[0]->name +"-"+ std::to_string(pl1[0]->ship_id));
		gui_renderer.player_name[1]->setText(pl1[1]->name +"-"+ std::to_string(pl1[1]->ship_id));
		...
		while (window.isOpen())
		{
			...
			for (int i = 0; i < pl1.size(); i++)
			{
				...
				if (mutx->m[pl1[i]->getMutexId(2369)].try_lock())
				{
					...
					if (pl1[i]->fuel>0 && l1.howMany() > 0 && l1.howMany() - 1 != pl1[i]->getPointPath(2369))
					{
						...
						pl1[i]->rect.setPosition(cx(l1[pl1[i]->getPointPath(2369)].x), cy(l1[pl1[i]->getPointPath(2369)].y));

						pl1[i]->update_tile_pos(pl1[i]->rect.getPosition().x - origin_x, pl1[i]->rect.getPosition().y - origin_y);
						...
						*/
						...
						if (pl1[0]->collide(pl1[1]->ship_id,pl1[i]->rect.getPosition().x,pl1[i]->rect.getPosition().y))
						{
							cout << "\n collision";
					...
					for (int j = 0; j <= pl1[i]->bullet_pointer; j++)
					{
						...
						{
							pl1[i]->cannon_ob.activeBullets[j].bullet_entity.setPosition(sf::Vector2f(pl1[i]->cannon_ob.activeBullets[j].bullet_trajectory[0].x + origin_x, pl1[i]->cannon_ob.activeBullets[j].bullet_trajectory[0].y + origin_y));
						...
						if (j >= 0)
						{
							...
							{
								//   cout << "\n1";
								Greed::coords coord((int)(pl1[i]->cannon_ob.activeBullets[j].bullet_entity.getPosition().y - origin_y) / len, (int)(pl1[i]->cannon_ob.activeBullets[j].bullet_entity.getPosition().x - origin_x) / len);
								// cout << "\n2";
							...
							if (pl1[i]->cannon_ob.activeBullets.size() > 0)
							{
								...
								Greed::coords cor = Greed::coords((int)(pl1[i]->cannon_ob.activeBullets[j].bullet_entity.getPosition().y - origin_y) / 80, (int)(pl1[i]->cannon_ob.activeBullets[j].bullet_entity.getPosition().x - origin_x) / 80);
								...
								if (!pl1[i]->cannon_ob.activeBullets[j].hit_or_not)
								{
									...
									for (int u = 0; u < 2; u++)
									{
										if (i != u && pl1[u]->died == 0)
										{
											if (pl1[i]->checkCollision(u, pl1[i]->cannon_ob.activeBullets[j]))
											{
												...
												// cout << "\n collision taken";
												check.push_back(pl1[i]->cannon_ob.activeBullets[j].id);
												found = 1;
												break;
											}
										}
									...
									for (int k = 0; k < cannon_list.howMany(); k++)
									{
										...
										if (cannon_list[k].tile == cor)
										{
											...
											pl1[i]->cannon_ob.activeBullets[j].hit_or_not = true;
											check1.push_back(pl1[i]->cannon_ob.activeBullets[j].id);
											found = 1;
											break;
										}
									}
							...
							if (pl1[i]->cannon_ob.activeBullets[j].hit_ship != -1)
							{
								...
								if (pl1[pl1[i]->cannon_ob.activeBullets[j].hit_ship]->health>0)
								{
									...
									animation_list.add_rear(animator(pl1[i]->cannon_ob.activeBullets[j].bullet_entity.getPosition(), elapsed_time, ANIMATION_TYPE::EXPLOSION, sf::Vector2f(pl1[pl1[i]->cannon_ob.activeBullets[j].hit_ship]->absolutePosition.x + origin_x, pl1[pl1[i]->cannon_ob.activeBullets[j].hit_ship]->absolutePosition.y + origin_y), pl1[i]->cannon_ob.activeBullets[j].hit_ship));
							...
							if (j>=0&&pl1[i]->cannon_ob.activeBullets[j].hit_cannon != -1 &&pl1[i]->cannon_ob.activeBullets[j].ttl == 15)//this condition is specially for the cannon
							{
								...
								*/
								...
								if (cannon_list[pl1[i]->cannon_ob.activeBullets[j].hit_cannon].health == 0)
								{
									...
									//pl1[i]->cannon_ob.allBullets[pl1[i]->cannon_ob.activeBullets[j].id].isActive = false;
									animation_list.add_rear(animator(pl1[i]->cannon_ob.activeBullets[j].bullet_entity.getPosition(), elapsed_time, ANIMATION_TYPE::EXPLOSION));
					...
					if (pl1[0]->rect.getGlobalBounds().intersects(pl1[1]->rect.getGlobalBounds()))
					{
			...
			for (int j = 0; j < cannon_list.howMany(); j++)
			{
				...
				for (int k = 0; k < cannon_list[j].bullet_list.size(); k++)
				{
					...
					if (cannon_list[j].bullet_list[k].bullet_trajectory.size() > 0)
					{
						...
						cannon_list[j].bullet_list[k].bullet_entity.setPosition(sf::Vector2f(cannon_list[j].bullet_list[k].bullet_trajectory[0].x + origin_x, cannon_list[j].bullet_list[k].bullet_trajectory[0].y + origin_y));
						for (int l = 1; l <= 3; l++)
						{
							if (cannon_list[j].bullet_list[k].bullet_trajectory.size() > 0)
							{
					...
					if (k >= 0)
					{
						...
						if (cannon_list[j].bullet_list.size() > 0)
						{
							...
							Greed::coords cor = Greed::coords((int)(cannon_list[j].bullet_list[k].bullet_entity.getPosition().y - origin_y) / 80, (int)(cannon_list[j].bullet_list[k].bullet_entity.getPosition().x - origin_x) / 80);
							// cout << "\n caught";
							// cout << "\n cor has==>" << cor.r << " " << cor.c;
					...
					if (cannon_list[j].bullet_list[k].ttl == 1)//here cannon bullet strikes the ship
					{
						...
						animation_list.add_rear(animator(cannon_list[j].bullet_list[k].bullet_entity.getPosition(), elapsed_time, ANIMATION_TYPE::EXPLOSION,sf::Vector2f((pl1[cannon_list[j].bullet_list[k].hit_ship]->absolutePosition.x+origin_x), pl1[cannon_list[j].bullet_list[k].hit_ship]->absolutePosition.y + origin_y),cannon_list[j].bullet_list[k].hit_ship));
						cannon_list[j].bullet_list.erase(cannon_list[j].bullet_list.begin());
						k--;
					}
			...
			for (int i = 0; i < 2; i++)//for drawing the bullet entity
			{
				for (int j = 0; j <= pl1[i]->bullet_pointer; j++)
				{
			...
			for (int i = 0; i < animation_list.howMany(); i++)
			{
				...
				if (animation_list[i].type == graphics::ANIMATION_TYPE::EXPLOSION)
				{
					if (animation_list[i].total_time <= animation_list[i].animation_time)
					{
						...
						{
							explosion_sprite.setPosition(animation_list[i].animation_position);
						}
...
int main()
{
	...
	player[1].initialize_player("anchit", &mutx, silist, code, 5, Greed::coords(8,15));//last element is the tile pos of the player
	player[0].initialize_player("player1", &mutx, silist, code, 5, Greed::coords(8, 9));
	...
	thread t(user1, ref(player[0]));
	thread t2(user2, ref(player[1]));
	graphics cg;
	t.detach();
	t2.detach();
