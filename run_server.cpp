...
void set_game()//to accept the connections and make the lobby
{
#if defined(_WIN32)
	WSADATA d;
	...
	double time_elapsed_limit = 10;//in minutes
	fd_set master;
	FD_ZERO(&master);
...
void graphics::callable(Mutex* mutx, int code[rows][columns], Map& map_ob)//taking the ship object so as to access the list of the player
{
	...
	const int no_of_players = pl1.size();
	...
	//socket programming starts from here
#if defined(_WIN32)
	WSADATA d;
	if (WSAStartup(MAKEWORD(2, 2), &d))
	{
	...
	FD_ZERO(&cur_sockets);
	FD_SET(socket_listen, &cur_sockets);
	SOCKET max_socket = socket_listen;
	...
	fd_set cur_socket;
	FD_ZERO(&cur_socket);
	FD_SET(socket_client, &cur_socket);
	int max_socket1 = socket_client;
	long double packet_no = 0;
	sf::Clock clock1;
	int p = 0;
	...
	while (1)
	{
		...
		sf::Time deltime1 = clock1.restart();
		tt += deltime1.asSeconds();
		frame_no = tt * frameRateLimit;
		total_secs += deltime1.asSeconds();
		
		elapsed_time += deltime1.asSeconds();
		if (tt > 1)
		{
			tt = 0;
		}
		if(frame_no!=val)//managing frame rate here
		{
			...
			//code to maintain the frame rate
			top_layer ship_packet;
			
			val = frame_no;
			fd_set temp = cur_socket;
			c++;
			if (t2 > 1 )
			{
			...
			*/
			...
			if (!gameOver)
			{
				...
				for (int i = 0; i < pl1.size(); i++)
				{
					...
					*/
						...
						if (pl1[i]->fuel > 0 && l1.howMany() > 0 && l1.howMany() - 1 != pl1[i]->getPointPath(2369))
						{
							...
							/*
							if ((abs(prev1.x - pl1[i]->absolutePosition.x) != 1 && abs(prev1.x - pl1[i]->absolutePosition.x) != 0) || (abs(prev1.y - pl1[i]->absolutePosition.y) != 0 && abs(prev1.y - pl1[i]->absolutePosition.y) != 1))
							{
								cout << "\n culprit found at==>" << prev1.x - pl1[i]->absolutePosition.x << " " << prev1.y - pl1[i]->absolutePosition.y;
							}
							prev1 = pl1[i]->absolutePosition;
							*/
						...
						for (int j = 0; j <= pl1[i]->bullet_pointer; j++)
						{
							...
							if (pl1[i]->cannon_ob.activeBullets[j].bullet_trajectory.size() > 0)
							{
								...
								pl1[i]->cannon_ob.activeBullets[j].absolute_position = pl1[i]->cannon_ob.activeBullets[j].bullet_trajectory[0];//updating the bullet_trajectory.
							...
							if (j >= 0)
							{
								...
								for (int k = 0; k < opaque.howMany(); k++)
								{
									...
									Greed::coords coord((int)((pl1[i]->cannon_ob.activeBullets[j].absolute_position.y) / len), (int)((pl1[i]->cannon_ob.activeBullets[j].absolute_position.x) / len));
								...
								if (pl1[i]->cannon_ob.activeBullets.size() > 0)
								{
									...
									if (!pl1[i]->cannon_ob.activeBullets[j].hit_or_not)
									{
										...
										for (int k = 0; k < cannon_list.howMany(); k++)
										{
											...
											//Greed::coords cor = Greed::coords((int)(::ry(pl1[i]->cannon_ob.activeBullets[j].bullet_entity.getPosition().y) - origin_y) / 80, (int)(::rx(pl1[i]->cannon_ob.activeBullets[j].bullet_entity.getPosition().x) - origin_x) / 80);
											Greed::coords cor = Greed::coords((int)((pl1[i]->cannon_ob.activeBullets[j].absolute_position.y) / len), (int)((pl1[i]->cannon_ob.activeBullets[j].absolute_position.x) / len));
								...
								if (pl1[i]->cannon_ob.activeBullets[j].hit_ship != -1)
								{
									...
									animation_list.add_rear(animator(sf::Vector2f(pl1[i]->cannon_ob.activeBullets[j].absolute_position.x + origin_x, pl1[i]->cannon_ob.activeBullets[j].absolute_position.y + origin_y), elapsed_time, ANIMATION_TYPE::EXPLOSION, sf::Vector2f(pl1[pl1[i]->cannon_ob.activeBullets[j].hit_ship]->absolutePosition.x + origin_x, pl1[pl1[i]->cannon_ob.activeBullets[j].hit_ship]->absolutePosition.y + origin_y), pl1[i]->cannon_ob.activeBullets[j].hit_ship));
								...
								if (j >= 0 && pl1[i]->cannon_ob.activeBullets[j].hit_cannon != -1 && pl1[i]->cannon_ob.activeBullets[j].ttl == 15)//this condition is specially for the cannon
								{
									...
									*/
									...
									animation_list.add_rear(animator(sf::Vector2f(pl1[i]->cannon_ob.activeBullets[j].absolute_position.x + origin_x, pl1[i]->cannon_ob.activeBullets[j].absolute_position.y + origin_y), elapsed_time, ANIMATION_TYPE::EXPLOSION));
				...
				for (int j = 0; j < cannon_list.howMany(); j++)
				{
					...
					for (int k = 0; k < cannon_list[j].bullet_list.size(); k++)
					{
						...
						if (cannon_list[j].bullet_list[k].bullet_trajectory.size() > 0)
						{
							...
							cannon_list[j].bullet_list[k].absolute_position = cannon_list[j].bullet_list[k].bullet_trajectory[0];
							for (int l = 1; l <= 3; l++)
							{
								if (cannon_list[j].bullet_list[k].bullet_trajectory.size() > 0)
								{
						...
						if (cannon_list[j].bullet_list[k].ttl == 1)//here cannon bullet strikes the ship
						{
							...
							animation_list.add_rear(animator(sf::Vector2f(cannon_list[j].bullet_list[k].absolute_position.x + origin_x, cannon_list[j].bullet_list[k].absolute_position.y + origin_y), elapsed_time, ANIMATION_TYPE::EXPLOSION, sf::Vector2f((pl1[cannon_list[j].bullet_list[k].hit_ship]->absolutePosition.x + origin_x), pl1[cannon_list[j].bullet_list[k].hit_ship]->absolutePosition.y + origin_y), cannon_list[j].bullet_list[k].hit_ship));
							cannon_list[j].bullet_list.erase(cannon_list[j].bullet_list.begin());
							k--;
						}
				...
				//making the packet of ship to send over the network
						//sending the timeline events over the wire
				...
				ship_packet.packet_no = packet_no;
				...
				for (int i = 0; i < pl1.size(); i++)
				{
					...
					for (int j =0; j < pl1[i]->killed_ships.size(); j++)
					{
					...
					for (int j = 0; j < pl1[i]->cannon_ob.activeBullets.size(); j++)
					{
						bullet_no++;
			...
			for (int i = 0; i < animation_list.howMany(); i++)
			{
				...
				if (animation_list[i].type == graphics::ANIMATION_TYPE::EXPLOSION)
				{
					...
					if (animation_list[i].total_time <= animation_list[i].animation_time)
					{
						...
						if (animation_list[i].sid != -1)//this is for the ship
						{
							...
							//explosion_sprite.setPosition(sf::Vector2f(cx(pos.x), cy(pos.y))); //to be  written in the client function
				...
				else if (animation_list[i].type == graphics::ANIMATION_TYPE::CANNON_FIRE)
				{
					...
					if (animation_list[i].total_time <= animation_list[i].animation_time)
					{
						...
						//send the cannon data with the angle
			...
			timeout.tv_sec = 0;
			timeout.tv_usec = 0;
			select(max_socket1 + 1, 0, &temp, 0, &timeout);
			if (FD_ISSET(socket_client, &temp))
			{
				...
				if (bytes < 1)
				{
					...
					FD_CLR(socket_client, &cur_sockets);
...
int main()
{
	...
	for (int i = 0; i < no_of_players; i++)
	{
		player[i].initialize_player("hello", &mutx, silist, code, 5, spawn[i]);//last element is the tile pos of the player
	...
	thread t[10];
		...
		t[0] = thread(user2::GreedMain, ref(player[0]));
		t[1] = thread(user3::GreedMain, ref(player[1]));
		t[2] = thread(user1::GreedMain, ref(player[2]));
